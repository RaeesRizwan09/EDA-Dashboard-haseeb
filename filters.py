...
def area_top_countries(df: pd.DataFrame, top_n: int = 8) -> plt.Figure:
    ...
    fig, ax = _new(11, 5.5)
    colors = plt.colormaps["YlOrBr"].resampled(len(top_c))
...
def box_by_year(df: pd.DataFrame) -> plt.Figure:
    ...
    cmap   = plt.colormaps["YlOrBr"].resampled(len(years))
    ...
    bp = ax.boxplot(data_by_year, patch_artist=True, orientation='vertical',
